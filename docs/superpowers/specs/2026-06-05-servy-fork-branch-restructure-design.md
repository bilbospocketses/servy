...
**Locked decisions:**
1. Fork-default branch name: **`develop`** (parallels velopack's integration branch; a `vnext`
   vision branch can later fork off `develop`, mirroring velopack, when todo `#2` starts).
...
BEFORE                                AFTER
------                                -----
main  (default, LOCKED)               develop  (default, LOCKED)   <- all fork work + releases
...
### 1. Branch model
- **`develop`** — GitHub default, carries ruleset `16655779` (deletion, non_fast_forward,
  required_linear_history, required_signatures, pull_request, required_status_checks =
  `Build on Windows` + `test`). All fork work + releases live here; feature branches fork off it.
- **`main`** — unprotected mirror of `upstream/main`, 0 ahead, synced at will. Never receives
  fork commits.

### 2. Catch-up strategy (192 commits)
Catch `develop` (currently at `origin/main`) up to `upstream/main` by **rebasing it onto
`upstream/main`** — dropping the empty smoke commit (`231c0a9b`) and **replaying only `bd913daa`**
(the rebrand) on top (e.g. `git rebase --onto upstream/main 231c0a9b develop`). Conflict resolution:
...
Result: `develop` is linear, current with upstream, rebrand on top, fully signed.

### 3. Ruleset + default migration
- **Retarget ruleset `16655779`** `refs/heads/main` -> `refs/heads/develop` (PUT; keep all
  rules incl. required checks). After this, `main` is unprotected and `develop` is locked.
- **Swap GitHub default** -> `develop`.
...
|---|---|---|
| `build`, `test` | push/PR `main` | **Retarget -> `develop`** (required checks; keeps the deadlock fixed on the new default) |
| `security` (CodeQL) | push/PR `main` + weekly | Retarget push/PR -> `develop`; keep schedule |
| `loc` | push `main` | Retarget -> `develop` |
...
1. Local hygiene: ensure `develop` exists from current `origin/main`; the spec/plan docs live here.
2. Catch up `develop`: rebase onto `upstream/main`, dropping the empty smoke commit, replaying
   the rebrand; resolve conflicts per Section 2.
3. Apply workflow retargets (Section 4) + commit on `develop` (signed, no AI attribution).
4. Push `develop`; set its upstream tracking to `origin/develop`.
5. Migrate ruleset `16655779` `main` -> `develop`; swap GitHub default -> `develop`.
...
The signature ordering works because `develop` is fully built + pushed **before** it is locked:
...
### 7. Post: memory + docs
- This spec + the writing-plans runbook committed on `develop`.
...
- `project_servy`: Fork Divergence (new default `develop`, workflow retargets, sonar disabled) +
  Lockdown State (ruleset `16655779` now targets `develop`).
...
- **Default-branch swap** changes clone/PR defaults to `develop` — intended.

## Out of scope
- Packaging rebrand (winget/choco/scoop manifests, `.iss`, signing) — todo `#4`, gated on fork
  releases.
- `vnext` vision branch — created later off `develop` when todo `#2` (initiative brainstorm) starts.
- Automated upstream-sync workflow — optional future enhancement.

## Verification (definition of done)
- `develop` is the GitHub default, locked by ruleset `16655779`; a smoke PR into `develop` runs
  and passes both `Build on Windows` + `test` (proving the trigger retarget).
...
- `develop` contains upstream's 192 commits + the rebrand, linear history, HEAD signed.
